...
- PDF 文本层翻译 MVP：对可复制文本的 PDF 提取页内文本，并在扩展阅读器中左右对照显示译文
...
- **PDF 阅读器翻译**：使用本地 `pdf.js` 解析文本层 PDF，按页提取文本块，并复用现有接入点、词库、预估和翻译链路。
...
5. 等待文本层解析完成后，可在阅读器内点击：
   ...
   - **Translate**：按页翻译并显示左右对照文本

第一版 PDF MVP 优先保证可读性，不尝试把译文覆盖到原 PDF 的精确坐标上。
...
- PDF MVP 以“按页原文/译文文本对照”为目标，不保证复刻原 PDF 的版式、双栏位置、图表位置或脚注位置。
